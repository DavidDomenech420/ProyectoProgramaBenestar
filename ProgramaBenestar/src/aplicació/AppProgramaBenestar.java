...
public class AppProgramaBenestar {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            String phrase="";
            phrase=fileRead.readLine(); //Llegim capçalera 1
            phrase=fileRead.readLine(); //Llegim capçalera 2
            phrase=fileRead.readLine(); //Llegim capçalera 3

            //Ara llegim primera línia amb informació:
            phrase=fileRead.readLine();
            
            Activities activity = null;
            while (phrase != null) {
                // Separem la frase per els ; i mirem el primer element
                String[] splitPhrase = phrase.split(";");
                String activityType = splitPhrase[0];
                // Agafem les primeres 6 parts ja que son iguals sense importar l'activitat
                String activityName = splitPhrase[1];
                LocalDate startInscriptionDate = LocalDate.of(Integer.parseInt(splitPhrase[2].split("-")[0]), Integer.parseInt(splitPhrase[2].split("-")[1]), Integer.parseInt(splitPhrase[2].split("-")[2]));
                LocalDate finishInscriptionDate = LocalDate.of(Integer.parseInt(splitPhrase[3].split("-")[0]), Integer.parseInt(splitPhrase[3].split("-")[1]), Integer.parseInt(splitPhrase[3].split("-")[2]));;
                String[] collectives = splitPhrase[4].split(",");
                int maxInscriptions = Integer.parseInt(splitPhrase[5]);
                
                // Segons el activityType mirarem uns parametres o altres
                if (activityType.equalsIgnoreCase("OneDay")) {
                    String city = splitPhrase[6];
                    LocalDate day = LocalDate.of(Integer.parseInt(splitPhrase[7].split("-")[0]), Integer.parseInt(splitPhrase[7].split("-")[1]), Integer.parseInt(splitPhrase[7].split("-")[2]));;
                    LocalTime startTime = LocalTime.of(Integer.parseInt(splitPhrase[8].split(":")[0]), Integer.parseInt(splitPhrase[8].split(":")[1]));
                    LocalTime finishTime = LocalTime.of(Integer.parseInt(splitPhrase[9].split(":")[0]), Integer.parseInt(splitPhrase[9].split(":")[1]));
                    double price = Double.parseDouble(splitPhrase[10]);
                    ...
                    String day = splitPhrase[6];
                    LocalTime startTime = LocalTime.of(Integer.parseInt(splitPhrase[7].split(":")[0]), Integer.parseInt(splitPhrase[7].split(":")[1]));
                    LocalTime finishTime = LocalTime.of(Integer.parseInt(splitPhrase[8].split(":")[0]), Integer.parseInt(splitPhrase[8].split(":")[1]));
                    LocalDate inicialDate = LocalDate.of(Integer.parseInt(splitPhrase[9].split("-")[0]), Integer.parseInt(splitPhrase[9].split("-")[1]), Integer.parseInt(splitPhrase[9].split("-")[2]));;
                    int weeksOfActivity = Integer.parseInt(splitPhrase[10]);
                    double price = Double.parseDouble(splitPhrase[11]);
                    String centerName = splitPhrase[12];
                    String cityName = splitPhrase[13];
                    ...
                    LocalDate startDateActivity = LocalDate.of(Integer.parseInt(splitPhrase[6].split("-")[0]), Integer.parseInt(splitPhrase[6].split("-")[1]), Integer.parseInt(splitPhrase[6].split("-")[2]));;
                    LocalDate finishDateActivity = LocalDate.of(Integer.parseInt(splitPhrase[7].split("-")[0]), Integer.parseInt(splitPhrase[7].split("-")[1]), Integer.parseInt(splitPhrase[7].split("-")[2]));;
                    String linkCourse = splitPhrase[8];
        ...
        try{
            ...
            String phrase= "OneDay;nameActivity;dd-mm-yyyy;dd-mm-yyyy;collective;29;city;dd-mm-yyyy;hh:mm;hh:mm;12.3";
            file.write(phrase);
            file.newLine();
            phrase="Periodic;nameActivity;dd-mm-yyyy;dd-mm-yyyy;collective;29;dd-mm-yyyy;hh:mm;dd-mm-yyyy;21;14.5;centerName;cityName";
            file.write(phrase);
            file.newLine();
            phrase = "Online;nameActivity;dd-mm-yyyy;dd-mm-yyyy;collective;29;dd-mm-yyyy;dd-mm-yyyy;linkCurso";
            file.write(phrase);
            file.newLine();
            ...
            for(int i=0; i<activities.getNumElems(); i++){
                file.newLine();
                phrase = "" + activities.getActivity(i);
                file.write(phrase);
            }
            file.close();
        } catch (FileNotFoundException e){
